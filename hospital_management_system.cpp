...
struct Single *head1 = NULL;
struct Double *head2 = NULL;
void insert_Single(char n[],char mNo[],string pId)
{
	
		struct Single *newNode=new struct Single;
...
void insert_Double(char n[],char mNo[],string pId)
{
	struct Double *newNode=new struct Double;
...
void del_Single(string pId)
{
	int flag=0;	
	struct Single *temp_del = head1;
...
void del_Double(string pId)
{
	
	int flag=0;	
	struct Double *temp_del = head2;
	if(temp_del->patientId==pId)
	{
		head2=head2->next;
		flag=1;
...
void billCalculation_Single(struct Single *temp_yes)
{
	int tenure;
...
void billCalculation_Double(struct Double *temp_yes)
{
	int tenure;
	cout<<"ENTER HIS STAY IN THE HOSPITAL(IN DAYS) : "<<endl;
	cin>>tenure;
	...
	cout<<"Incharge Doctor : "<<temp_yes->doc_name<<endl;
    cout<<"\n---------------------COST DETAILS---------------------"<<endl;
	cout<<"HOSPITAL FEES : "<<(5000*tenure)<<"\n"<<endl;
	cout<<"MEDICAL FEES : "<<(500*tenure)<<"\n"<<endl;
	for(int i=0;i<15;i++)
	{
		if(temp_yes->doc_name == doc[i].name)
		{ 
			cout<<"DOCTOR FEES : "<<(doc[i].fees * tenure)<<endl;
			cout<<"TAX PAYABLE : "<<0.05*((5000*tenure)+(500*tenure)+(doc[i].fees*tenure))<<endl;
			cout<<"--------------------------------------------------------------"<<endl;
			cout<<"TOTAL FEES : "<<1.05*((5000*tenure)+(500*tenure)+(doc[i].fees*tenure))<<endl;
			cout<<"--------------------------------------------------------------"<<endl;
			break;
		}
	}				    
...
void sort_single()
{
 	struct Single  *i = head1;

 	while(i->next!=NULL)
	{
		struct Single  *ptr = i;
		struct Single  *j = i->next;
...
void sort_double()
{
 	struct Double  *i = head2;

 	while(i->next!=NULL)
	{
		struct Double  *ptr = i;
		struct Double  *j = i->next;
...
int main()
{
	...
	if(choice==1)
	{
		...
		cin>>patientId;
			if(patientId.length()!=4 || s_search(patientId)==1 || d_search(patientId)==1)
				cout<<"Invalid Patient ID : "<<endl;
		}while(patientId.length()!=4 || s_search(patientId)==1 || d_search(patientId)==1);
			...
			case 1:	if(COUNT_S==5)
					{
						...
						insert_Double(name,mobNo,patientId);
						COUNT_D++;
						choose=2;	
					}
					else
					{
						if(choose!=2)
							COUNT_S++;
						insert_Single(name,mobNo,patientId);
			...
			if(yn=='Y' || yn=='y')
			{
				...
				while(temp_yes->next!=NULL)
				{
					if(temp_yes->patientId == discharge)
					{
		    			...
		    			//temp->next=newNode;
		    			billCalculation_Single(temp_yes);
						found=1;
		    			break;
					}
					else
			...
			if(yn=='Y' || yn=='y')
			{
				...
				while(temp_yes->next!=NULL)
				{
					if(temp_yes->patientId == discharge)
					{
						...
						billCalculation_Double(temp_yes);
		  			    found=1;
		    			break;
					}
					else
